...
Запросы к API замоканы через `page.route` (`e2e/fixtures.ts`), поэтому тесты не зависят от
доступности бэкенда и всегда детерминированы. Авторизация подставляется токеном в
`localStorage`: страницы входа и регистрации намеренно не участвуют в сценарии создания
заказа. Dev-сервер поднимается автоматически (`webServer` в `playwright.config.ts`).
